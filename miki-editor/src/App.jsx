...
function AppContent() {
  ...
  const [isSetup, setIsSetup] = useState(() => {
    try { return localStorage.getItem('miki_is_setup') === 'true'; } catch { return false; }
  });
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          // 404 오류 시 문서 목록 새로고침하여 동기화
          logger.warn(`문서 ${id}를 찾을 수 없음, 문서 목록 새로고침`);
          refetchDocuments(); // React Query로 문서 목록 새로고침
          throw new Error(`문서 '${id}'를 찾을 수 없습니다. 이미 삭제되었거나 존재하지 않는 문서입니다.`);
        }
        throw new Error(`문서 로드 실패: ${response.status} ${response.statusText}`);
      }
  ...
  if (!isSetup) {
    return (
      <SetupWizard onComplete={() => { try { localStorage.setItem('miki_is_setup', 'true'); } catch {}; setIsSetup(true); }} />
    );
  }
